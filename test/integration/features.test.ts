...
describe("Agent update (PUT)", () => {
  ...
  it("updates name", async () => {
    const res = await put(`/v1/oma/agents/${agentId}`, { name: "Updated Name" });
  ...
  it("increments version on update", async () => {
    const before = (await (await get(`/v1/oma/agents/${agentId}`)).json()) as any;
    await put(`/v1/oma/agents/${agentId}`, { system: "updated system" });
    const after = (await (await get(`/v1/oma/agents/${agentId}`)).json()) as any;
    expect(after.version).toBe(before.version + 1);
  });

  it("updates model", async () => {
    const res = await put(`/v1/oma/agents/${agentId}`, { model: "claude-opus-4-6" });
    const body = (await res.json()) as any;
    expect(body.model.id).toBe("claude-opus-4-6");
  });

  it("updates tools", async () => {
    const res = await put(`/v1/oma/agents/${agentId}`, {
  ...
  it("updates description", async () => {
    const res = await put(`/v1/oma/agents/${agentId}`, { description: "A test agent" });
    const body = (await res.json()) as any;
    expect(body.description).toBe("A test agent");
  });

  it("returns 404 for nonexistent agent", async () => {
    const res = await put("/v1/oma/agents/agent_ghost", { name: "x" });
...
describe("Agent delete (DELETE)", () => {
  it("deletes an agent", async () => {
    const createRes = await post("/v1/oma/agents", { name: "ToDelete", model: "claude-sonnet-4-6", harness: "noop-test" });
  ...
  it("returns 404 for nonexistent agent", async () => {
    const res = await del("/v1/oma/agents/agent_ghost");
    expect(res.status).toBe(404);
  });
});
...
describe("Agent archive", () => {
  it("archives an agent", async () => {
    const createRes = await post("/v1/oma/agents", { name: "ToArchive", model: "claude-sonnet-4-6", harness: "noop-test" });
    const agent = (await createRes.json()) as any;
    ...
    // Verify persisted
    const getRes = await get(`/v1/oma/agents/${archived.id}`);
    const fetched = (await getRes.json()) as any;
    expect(fetched.archived_at).toBeTruthy();
  });

  it("returns 404 for nonexistent agent", async () => {
    const res = await post("/v1/oma/agents/agent_ghost/archive", {});
    expect(res.status).toBe(404);
  });
});
...
describe("Agent version history", () => {
  ...
  beforeAll(async () => {
    const res = await post("/v1/oma/agents", { name: "Versioned", model: "claude-sonnet-4-6", system: "v1", harness: "noop-test" });
    agentId = ((await res.json()) as any).id;
    // Create 3 versions
    await put(`/v1/oma/agents/${agentId}`, { system: "v2" });
    await put(`/v1/oma/agents/${agentId}`, { system: "v3" });
  });

  it("lists all versions", async () => {
    const res = await get(`/v1/oma/agents/${agentId}/versions`);
    expect(res.status).toBe(200);
    const body = (await res.json()) as any;
  ...
  it("gets a specific version", async () => {
    const res = await get(`/v1/oma/agents/${agentId}/versions/1`);
    expect(res.status).toBe(200);
    const body = (await res.json()) as any;
  ...
  it("current version is v3", async () => {
    const res = await get(`/v1/oma/agents/${agentId}`);
    const body = (await res.json()) as any;
  ...
  it("returns 404 for nonexistent version", async () => {
    const res = await get(`/v1/oma/agents/${agentId}/versions/99`);
    expect(res.status).toBe(404);
  });
});
...
describe("Agent list filters", () => {
  beforeAll(async () => {
    // Create a few agents
    for (let i = 0; i < 3; i++) {
      await post("/v1/oma/agents", { name: `ListTest-${i}`, model: "claude-sonnet-4-6", harness: "noop-test" });
    }
  });
  ...
  it("limits results", async () => {
    const res = await get("/v1/oma/agents?limit=2");
    const body = (await res.json()) as any;
    expect(body.data.length).toBe(2);
  });

  it("orders ascending", async () => {
    const res = await get("/v1/oma/agents?order=asc&limit=50");
    const body = (await res.json()) as any;
...
describe("Environment update/delete/archive", () => {
  ...
  beforeAll(async () => {
    const res = await post("/v1/oma/environments", { name: "updatable-env", config: { type: "cloud" } });
    envId = ((await res.json()) as any).id;
  });

  it("updates environment name", async () => {
    const res = await put(`/v1/oma/environments/${envId}`, { name: "renamed-env" });
    expect(res.status).toBe(200);
    const body = (await res.json()) as any;
  ...
  it("updates environment config", async () => {
    const res = await put(`/v1/oma/environments/${envId}`, {
  ...
  it("archives environment", async () => {
    const envRes = await post("/v1/oma/environments", { name: "to-archive", config: { type: "cloud" } });
    const env = (await envRes.json()) as any;

    const archRes = await post(`/v1/oma/environments/${env.id}/archive`, {});
    expect(archRes.status).toBe(200);
    const archived = (await archRes.json()) as any;
    expect(archived.archived_at).toBeTruthy();
  });
  ...
  it("deletes environment", async () => {
    const envRes = await post("/v1/oma/environments", { name: "to-delete", config: { type: "cloud" } });
    const env = (await envRes.json()) as any;

    const delRes = await del(`/v1/oma/environments/${env.id}`);
    expect(delRes.status).toBe(200);
    expect(((await delRes.json()) as any).type).toBe("environment_deleted");

    const getRes = await get(`/v1/oma/environments/${env.id}`);
    expect(getRes.status).toBe(404);
  });

  it("returns 404 for update/delete/archive on nonexistent env", async () => {
    expect((await put("/v1/oma/environments/env_ghost", { name: "x" })).status).toBe(404);
    expect((await del("/v1/oma/environments/env_ghost")).status).toBe(404);
    expect((await post("/v1/oma/environments/env_ghost/archive", {})).status).toBe(404);
  });
});
...
describe("Session update/delete/archive", () => {
  ...
  beforeAll(async () => {
    const a = await post("/v1/oma/agents", { name: "SA", model: "claude-sonnet-4-6", harness: "noop-test" });
    agentId = ((await a.json()) as any).id;
    const e = await post("/v1/oma/environments", { name: "se", config: { type: "cloud" } });
    envId = ((await e.json()) as any).id;
    const s = await post("/v1/oma/sessions", { agent: agentId, environment_id: envId, title: "Original" });
    sessionId = ((await s.json()) as any).id;
  });

  it("updates session title", async () => {
    const res = await post(`/v1/oma/sessions/${sessionId}`, { title: "Updated Title" });
    expect(res.status).toBe(200);
    const body = (await res.json()) as any;
  ...
  it("updates session metadata", async () => {
    const res = await post(`/v1/oma/sessions/${sessionId}`, {
  ...
  it("archives session", async () => {
    ...
    const archRes = await post(`/v1/oma/sessions/${sess.id}/archive`, {});
    expect(archRes.status).toBe(200);
  ...
  it("deletes session", async () => {
    ...
    const delRes = await del(`/v1/oma/sessions/${sess.id}`);
    expect(delRes.status).toBe(200);
    expect(((await delRes.json()) as any).type).toBe("session_deleted");

    expect((await get(`/v1/oma/sessions/${sess.id}`)).status).toBe(404);
  });
});
...
describe("Session list filters", () => {
  ...
  beforeAll(async () => {
    const a1 = await post("/v1/oma/agents", { name: "Filter1", model: "claude-sonnet-4-6", harness: "noop-test" });
    agentId1 = ((await a1.json()) as any).id;
    const a2 = await post("/v1/oma/agents", { name: "Filter2", model: "claude-sonnet-4-6", harness: "noop-test" });
    agentId2 = ((await a2.json()) as any).id;
    const e = await post("/v1/oma/environments", { name: "filt-env", config: { type: "cloud" } });
    envId = ((await e.json()) as any).id;

    // Create 2 sessions for agent1, 1 for agent2
    await post("/v1/oma/sessions", { agent: agentId1, environment_id: envId, title: "S1A1" });
    await post("/v1/oma/sessions", { agent: agentId1, environment_id: envId, title: "S2A1" });
    await post("/v1/oma/sessions", { agent: agentId2, environment_id: envId, title: "S1A2" });
    // Archive one
    const archSess = await post("/v1/oma/sessions", { agent: agentId1, environment_id: envId, title: "Archived" });
    const archId = ((await archSess.json()) as any).id;
    await post(`/v1/oma/sessions/${archId}/archive`, {});
  });

  it("filters by agent_id", async () => {
    const res = await get(`/v1/oma/sessions?agent_id=${agentId1}`);
    const body = (await res.json()) as any;
  ...
  it("includes archived when requested", async () => {
    const res = await get(`/v1/oma/sessions?agent_id=${agentId1}&include_archived=true`);
    const body = (await res.json()) as any;
  ...
  it("limits results", async () => {
    const res = await get("/v1/oma/sessions?limit=1");
    const body = (await res.json()) as any;
...
describe("New event types", () => {
  ...
  beforeAll(async () => {
    const a = await post("/v1/oma/agents", { name: "Evt", model: "claude-sonnet-4-6", harness: "echo-test" });
    const e = await post("/v1/oma/environments", { name: "evt-env", config: { type: "cloud" } });
...
describe("Events pagination", () => {
  ...
  beforeAll(async () => {
    const a = await post("/v1/oma/agents", { name: "Pag", model: "claude-sonnet-4-6", harness: "noop-test" });
    const e = await post("/v1/oma/environments", { name: "pag-env", config: { type: "cloud" } });
    const s = await post("/v1/oma/sessions", {
      agent: ((await a.json()) as any).id,
      environment_id: ((await e.json()) as any).id,
    });
...
describe("Session agent snapshot", () => {
  it("session stores agent snapshot at creation time", async () => {
    ...
    await put(`/v1/oma/agents/${agent.id}`, { system: "snapshot v2" });
...
describe("Session harness status flow", () => {
  it("emits running → idle events during processing", async () => {
    const a = await post("/v1/oma/agents", { name: "Status", model: "claude-sonnet-4-6", harness: "echo-test" });
    const e = await post("/v1/oma/environments", { name: "stat-env", config: { type: "cloud" } });
    const s = await post("/v1/oma/sessions", {
      agent: ((await a.json()) as any).id,
      environment_id: ((await e.json()) as any).id,
    });
  ...
  it("idle event has stop_reason", async () => {
    const a = await post("/v1/oma/agents", { name: "StopR", model: "claude-sonnet-4-6", harness: "echo-test" });
    const e = await post("/v1/oma/environments", { name: "stop-env", config: { type: "cloud" } });
    const s = await post("/v1/oma/sessions", {
      agent: ((await a.json()) as any).id,
      environment_id: ((await e.json()) as any).id,
    });
...
describe("Harness crash recovery", () => {
  it("emits session.error on crash and returns to idle", async () => {
    ...
    const a = await post("/v1/oma/agents", { name: "Crash", model: "claude-sonnet-4-6", harness: "crash-v2" });
    const e = await post("/v1/oma/environments", { name: "crash-env", config: { type: "cloud" } });
    const s = await post("/v1/oma/sessions", {
      agent: ((await a.json()) as any).id,
      environment_id: ((await e.json()) as any).id,
    });
