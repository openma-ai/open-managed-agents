...
If your deployment outgrows a single D1 (D1 has a 10 GB / 50K-row limits per database), you can fan out across N AUTH_DB shards. The shard router lives in `ROUTER_DB`; per-tenant data lands on whichever shard `tenant_shard` says owns the tenant.
