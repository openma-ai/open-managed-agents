...
For the ACP supervisor, `driver.harness` selects a published software release,
for example `{ id: "codex-acp", version: "1.8.0" }`. The environment supplies
Node, npm and the generic supervisor; codex-acp does not need to be baked into
its image. The supervisor resolves and verifies the exact npm artifact through
`@openma/common/acp-artifacts`, prepares it in an isolated version directory,
and then starts it. Allow first-install time in `readyTimeoutMs` (for example
660,000 ms). A cold sandbox needs access to npm and its artifact downloads.

`OPENMA_ACP_PACKAGES` optionally replaces the default codex-acp package mapping
with a JSON object of harness ids to npm package names. Release versions come
from package metadata. The Session's resolved manifest travels with workspace
snapshots; native checkpoints bind the artifact digest. Another version or
artifact requires a new Session. This path currently supports npm packages;
fully reproducible transitive dependencies require bundled/shrinkwrapped
releases. Existing preinstalled inventories (`OPENMA_ACP_HARNESSES`) and legacy
unpinned `version: "1"` remain available for existing deployments. The wire
protocol remains `openma-harness-supervisor-v1`.
