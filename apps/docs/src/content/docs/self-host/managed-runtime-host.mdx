...
For the ACP supervisor, `driver.harness` selects an exact installed software
release, for example `{ id: "codex-acp", version: "1.8.0" }`. Declare the
image's installed executables in `OPENMA_ACP_HARNESSES` (a JSON array of
`{ id, version, command, args? }`, with absolute command paths). The supervisor
matches both fields and rejects missing versions. The inventory is image-owned;
do not generate it from the requested version. Without it, `version: "1"`
retains legacy, unpinned registry selection. The protocol remains
`openma-harness-supervisor-v1`. Versioned native checkpoints cannot be resumed
with another harness release; use a new Session when upgrading.
