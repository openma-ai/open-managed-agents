...
| Caller | Knows | Asks |
|---|---|---|
| Local-runtime ACP child | `(sid, server, agent api key)` | HTTP `/v1/oma/mcp-proxy/<sid>/<server>` with `Authorization: Bearer <apiKey>` |
...
- Internal architecture deep-dive: [`docs/mcp-credential-architecture.md`](https://github.com/openma-ai/open-managed-agents/blob/main/docs/mcp-credential-architecture.md)
- E2E test fixture: [`test/fixtures/fake-oauth-mcp/`](https://github.com/openma-ai/open-managed-agents/tree/main/test/fixtures/fake-oauth-mcp) — a deployable CF Worker that plays both OAuth provider and MCP server, used to drill the refresh path without burning real-provider credentials.
